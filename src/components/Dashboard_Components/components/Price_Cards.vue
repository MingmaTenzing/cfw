...
import {change_and_prediction} from '../../../../utils/price_change_and_prediction'
const cheapest_site = ref<fuelwatch_xml[]>([])
const average_unleaded = ref<number>()
const change_and_prediction = ref<price_change_predict>()

const site_loading = ref<boolean>(false)

onMounted(async () => {

  const [cheapest_station, average_data, price_change_and_predict] = await Promise.all([
  ...
  change_and_prediction.value = price_change_and_predict

console.log(cheapest_site.value,average_unleaded.value, change_and_prediction.value)
})
...
<template>
  ...
  <div
      class="border w-[325px] lg:w-1/3 p-4 rounded-lg space-y-2   "
    >
      <div class="space-y-4">
        <div>
        ...
        <div class=" text-end">
          ...
          <p class="font-bold text-3xl xl:text-4xl text-green-600">${{((cheapest_site[0]?.price)/100).toFixed(2)}}/L</p>
        </div>
      </div>
    </div>
    ...
    >
      <div class="space-y-4">
        ...
        <div class=" text-end">
          ...
          <p class="font-bold text-3xl xl:text-4xl text-green-600">${{((cheapest_site[1]?.price)/100).toFixed(2)}}/L</p>
        </div>
      </div>
    </div>
