...
export const change_and_prediction = async () => {
  const [yesterday_prices, today_prices] = await Promise.all([
    axios.get(
      'https://api.allorigins.win/raw?url=https://www.fuelwatch.wa.gov.au/fuelwatch/fuelWatchRSS?Day=yesterday&Region=26',
    ),
    axios.get(
      'https://api.allorigins.win/raw?url=https://www.fuelwatch.wa.gov.au/fuelwatch/fuelWatchRSS?Day=today&Region=26',
    ),
  ])
  ...
  const yesterday_average =
    parse_xml_yesterday.reduce((sum, price) => sum + price.price, 0) / parse_xml_yesterday.length
  ...
  return { percentage_change: percentage_change, tomorrow_predicted_price: tomorrow_prediction }
}
